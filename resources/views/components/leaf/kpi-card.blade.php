...
>
    <div class="mb-4 flex min-w-0 items-start justify-between gap-3">
        <div class="min-w-0 space-y-2">
            ...
            <div class="flex items-baseline gap-1.5 min-w-0">
                <span class="truncate text-2xl font-extrabold tracking-tight text-[#1B4332] transition-all duration-300 ease-out"
                    @if ($sensorKey)
        ...
        @elseif ($sensorKey)
            <span class="inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.18em] transition-colors duration-300"
                x-bind:class="kpiStatusBadgeClass('{{ $sensorKey }}')"
            >
                <span class="h-1.5 w-1.5 rounded-full" x-bind:class="kpiStatusDotClass('{{ $sensorKey }}')"></span>
    ...
    <div class="mt-4 flex items-center justify-between gap-3 border-t border-[#2D6A4F]/10 pt-3 text-[11px]">
        ...
        @elseif ($icon)
            <x-leaf.status-badge :type="$statusType" :label="$status" class="shrink-0" />
        @else
            <span class="truncate font-semibold text-[#2D6A4F]"
                @if ($sensorKey)
